...
const PersonalDetails = ({ personalDetails }) => {
  ...
  const { name, address, phone, email } = personalDetails
  return (
    <header className='PersonalDetails bg-theme text-white p-3'>
      <h1 className='name text-uppercase'>{name}</h1>
...
const Experience = ({ title, experiences }) => {
  ...
  return (
    <section className='mt-3'>
      <SectionHeading>{title}</SectionHeading>
      <div className="row">
        {experiences && experiences.map((ex, idx) => (
          <div className='col-12' key={idx}>
            <ul>
              <li><b>{ex.companyName}</b></li>
              <p className='fs-7 mb-0'><em>{ex.startDate} - {ex.endDate}</em></p>
...
function MBase4({ templateId, profile }) {
    ...
    <div className={templateId}>
      <div className='row'>
        <div className="col-8">
          ...
          </Projects>         <Experience title='Experience' experiences={experiences} />
          <AdditionalInformation title='Additional Information' additionalInformation={additionalInformation} />
        </div>
        <div className='col-4 right-column position-relative'>
          <div className='position-absolute top-0 bottom-0 bg-theme' style={{ left: 0, right: 0, opacity: 0.1 }} />
          <div className='d-flex align-items-center justify-content-center'>
            {personalDetails?.profileImage && (
              <div className='profile-img my-4'>
                <img src={personalDetails?.profileImageBase64} alt="profile image" />
              </div>
            )}
          </div>
